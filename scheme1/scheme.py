...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        try:
            ...
            eval_rest = rest.map(lambda x: scheme_eval(x, env))
            return scheme_apply(procedure, eval_rest, env)
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("number of arguments does not match with the number of formal parameters")
        if len(formals) != len(vals):
            raise SchemeError
        form, val = formals, vals
        while form != nil:
            child.bindings[form.first] = val.first
            form, val = form.second, val.second
        return child
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            return scheme_eval(clause.second, env, True)
            # END PROBLEM 14
        expressions = expressions.second
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        if not scheme_symbolp(current.first):
            raise SchemeError("A value must be bound to a symbol")
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result, Thunk):
            ...
            if scheme_symbolp(first) and (first in SPECIAL_FORMS):
                ...
                first, rest = original_scheme_eval(first, env), rest.map(lambda x: original_scheme_eval(x, env))
                check_procedure(first)
                result = scheme_apply(first, rest, env)
