...
def balanced(m):
    ...
    is_torque_balanced = total_weight(end(left_side)) * length(left_side) == total_weight(end(right_side)) * length(right_side)
    is_sub_balanced = balanced(end(left_side)) and balanced(end(right_side))
...
def make_counter():
    ...
    counter_dict = {}
    def counter(x):
    	if x not in counter_dict.keys():
    		counter_dict[x] = 1
    	else:
    		counter_dict[x] += 1
    	return counter_dict[x]
...
def make_joint(withdraw, old_password, new_password):
    ...
    def withdraw_joint(amount, passcode):
    	if passcode in [old_password, new_password] :
