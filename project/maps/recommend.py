...
def best_predictor(user, restaurants, feature_fns):
    ...
    for feature in feature_fns:
        a, b  = find_predictor(user, reviewed, feature)
        predictor.append(a)
        r_squared.append(b)

    best = max(zip(predictor,r_squared), key = lambda x: x[1])
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        if restaurant in reviewed:
            rate_dictionary[restaurant_name(restaurant)] = user_rating(user, restaurant_name(restaurant))
        else:
            rate_dictionary[restaurant_name(restaurant)] = predictor(restaurant)
