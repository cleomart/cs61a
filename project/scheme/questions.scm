...
(define (enumerate s)
  ...
  (define (enumerate-index i s) 
    (if (null? s) nil
      (cons (list i (car s)) (enumerate-index (+ i 1) (cdr s))))
    )
  (enumerate-index 0 s)
  )
  ; END PROBLEM 17
...
(define (list-change total denoms)
  ...
  (cond 
    ...
    ((= total (car denoms)) (append (list-change 0 denoms) (list-change total (cdr denoms))))
        ...
        ((or (lambda? expr)
         ...
         (let ((form   (car expr))
           ...
           (if (null? (cdr (cddr expr)))
            ...
            (list form params (car body) (let-to-lambda (cadr (cddr expr)))))
           ; END PROBLEM 19
           ))
        ((let? expr)
